  ...
  methods: {
      ...
      if (result) {
        this.shares[index].like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://floating-castle-12791.herokuapp.com/api/like",
              data: {
        ...
        axios
          .post("https://floating-castle-12791.herokuapp.com/api/like", {
            share_id: this.shares[index].item.id,
            user_id: this.$store.state.user.id,
          })
          .then((response) => {
    ...
    async getShares() {
      ...
      const shares = await axios.get(
        "https://floating-castle-12791.herokuapp.com/api/shares"
      );
